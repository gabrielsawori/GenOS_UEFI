/*
 * AES-256-CBC — FIPS 197 Advanced Encryption Standard
 *
 * Pure C implementation for bare-metal (no libc dependency).
 * AES-256 uses 14 rounds with a 256-bit (32-byte) key.
 * CBC mode chains blocks with XOR of previous ciphertext.
 * PKCS#7 padding ensures plaintext is block-aligned.
 *
 * This is a straightforward T-table-free implementation
 * (smaller code size, constant-time-ish on most CPUs).
 */
#include "aes.h"

/* AES S-Box (substitution box) */
static const uint8_t sbox[256] = {
    
};

/* Inverse S-Box (for decryption) */
static const uint8_t inv_sbox[256] = {
    
};

/* Round constant for key expansion */
static const uint8_t rcon[10] = {
    0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36
};

/* GF(2^8) multiplication helpers for MixColumns */
static uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

static uint8_t gf_mul(uint8_t a, uint8_t b) {
    uint8_t p = 0;
    for (int i = 0; i < 8; i++) {
        if (b & 1) p ^= a;
        uint8_t hi = a & 0x80;
        a <<= 1;
        if (hi) a ^= 0x1b;
        b >>= 1;
    }
    return p;
}

/* AES-256 key expansion: 32-byte key → 15 round keys (240 bytes) */
static void key_expansion(const uint8_t key[32], uint8_t rk[240]) {
    /* Copy original key as first 8 words */
    for (int i = 0; i < 32; i++) rk[i] = key[i];

    int bytes_generated = 32;
    int rcon_idx = 0;

    while (bytes_generated < 240) {
        /* Take last 4 bytes */
        uint8_t tmp[4];
        tmp[0] = rk[bytes_generated - 4];
        tmp[1] = rk[bytes_generated - 3];
        tmp[2] = rk[bytes_generated - 2];
        tmp[3] = rk[bytes_generated - 1];

        if (bytes_generated % 32 == 0) {
            /* RotWord + SubWord + Rcon */
            uint8_t t = tmp[0];
            tmp[0] = sbox[tmp[1]] ^ rcon[rcon_idx++];
            tmp[1] = sbox[tmp[2]];
            tmp[2] = sbox[tmp[3]];
            tmp[3] = sbox[t];
        } else if (bytes_generated % 32 == 16) {
            /* SubWord only (AES-256 extra step) */
            tmp[0] = sbox[tmp[0]];
            tmp[1] = sbox[tmp[1]];
            tmp[2] = sbox[tmp[2]];
            tmp[3] = sbox[tmp[3]];
        }

        for (int i = 0; i < 4; i++)
            rk[bytes_generated + i] = rk[bytes_generated - 32 + i] ^ tmp[i];
        bytes_generated += 4;
    }
}

/* AddRoundKey: XOR state with round key */
static void add_round_key(uint8_t state[16], const uint8_t* rk) {
    for (int i = 0; i < 16; i++) state[i] ^= rk[i];
}

/* SubBytes: substitute each byte via S-Box */
static void sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = sbox[state[i]];
}

/* InvSubBytes */
static void inv_sub_bytes(uint8_t state[16]) {
    for (int i = 0; i < 16; i++) state[i] = inv_sbox[state[i]];
}

/* ShiftRows: cyclic left-shift rows 1,2,3 by 1,2,3 positions */
static void shift_rows(uint8_t state[16]) {
    uint8_t t;
    /* Row 1: shift left 1 */
    t = state[1]; state[1] = state[5]; state[5] = state[9]; state[9] = state[13]; state[13] = t;
    /* Row 2: shift left 2 */
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    /* Row 3: shift left 3 (= right 1) */
    t = state[15]; state[15] = state[11]; state[11] = state[7]; state[7] = state[3]; state[3] = t;
}

/* InvShiftRows */
static void inv_shift_rows(uint8_t state[16]) {
    uint8_t t;
    /* Row 1: shift right 1 */
    t = state[13]; state[13] = state[9]; state[9] = state[5]; state[5] = state[1]; state[1] = t;
    /* Row 2: shift right 2 */
    t = state[2]; state[2] = state[10]; state[10] = t;
    t = state[6]; state[6] = state[14]; state[14] = t;
    /* Row 3: shift right 3 (= left 1) */
    t = state[3]; state[3] = state[7]; state[7] = state[11]; state[11] = state[15]; state[15] = t;
}

/* MixColumns: column-wise matrix multiply in GF(2^8) */
static void mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        int i = c * 4;
        uint8_t a0 = state[i], a1 = state[i+1], a2 = state[i+2], a3 = state[i+3];
        state[i]   = xtime(a0) ^ xtime(a1) ^ a1 ^ a2 ^ a3;
        state[i+1] = a0 ^ xtime(a1) ^ xtime(a2) ^ a2 ^ a3;
        state[i+2] = a0 ^ a1 ^ xtime(a2) ^ xtime(a3) ^ a3;
        state[i+3] = xtime(a0) ^ a0 ^ a1 ^ a2 ^ xtime(a3);
    }
}

/* InvMixColumns */
static void inv_mix_columns(uint8_t state[16]) {
    for (int c = 0; c < 4; c++) {
        int i = c * 4;
        uint8_t a0 = state[i], a1 = state[i+1], a2 = state[i+2], a3 = state[i+3];
        state[i]   = gf_mul(a0, 0x0e) ^ gf_mul(a1, 0x0b) ^ gf_mul(a2, 0x0d) ^ gf_mul(a3, 0x09);
        state[i+1] = gf_mul(a0, 0x09) ^ gf_mul(a1, 0x0e) ^ gf_mul(a2, 0x0b) ^ gf_mul(a3, 0x0d);
        state[i+2] = gf_mul(a0, 0x0d) ^ gf_mul(a1, 0x09) ^ gf_mul(a2, 0x0e) ^ gf_mul(a3, 0x0b);
        state[i+3] = gf_mul(a0, 0x0b) ^ gf_mul(a1, 0x0d) ^ gf_mul(a2, 0x09) ^ gf_mul(a3, 0x0e);
    }
}

void aes256_encrypt_block(const uint8_t key[32],
                          const uint8_t in[16], uint8_t out[16])
{
    uint8_t rk[240], state[16];
    key_expansion(key, rk);

    for (int i = 0; i < 16; i++) state[i] = in[i];

    add_round_key(state, rk);

    /* Rounds 1-13 */
    for (int r = 1; r <= 13; r++) {
        sub_bytes(state);
        shift_rows(state);
        mix_columns(state);
        add_round_key(state, rk + r * 16);
    }

    /* Round 14 (no MixColumns) */
    sub_bytes(state);
    shift_rows(state);
    add_round_key(state, rk + 14 * 16);

    for (int i = 0; i < 16; i++) out[i] = state[i];
}

void aes256_decrypt_block(const uint8_t key[32],
                          const uint8_t in[16], uint8_t out[16])
{
    uint8_t rk[240], state[16];
    key_expansion(key, rk);

    for (int i = 0; i < 16; i++) state[i] = in[i];

    add_round_key(state, rk + 14 * 16);

    /* Rounds 13-1 (inverse) */
    for (int r = 13; r >= 1; r--) {
        inv_shift_rows(state);
        inv_sub_bytes(state);
        add_round_key(state, rk + r * 16);
        inv_mix_columns(state);
    }

    /* Round 0 */
    inv_shift_rows(state);
    inv_sub_bytes(state);
    add_round_key(state, rk);

    for (int i = 0; i < 16; i++) out[i] = state[i];
}

int aes256_cbc_encrypt(const uint8_t key[32], const uint8_t iv[16],
                       const void* plaintext, size_t pt_len,
                       void* out_buf, size_t out_max)
{
    const uint8_t* pt = (const uint8_t*)plaintext;
    uint8_t* ct = (uint8_t*)out_buf;

    /* Calculate padded length (PKCS#7: always add at least 1 byte) */
    size_t padded_len = ((pt_len / 16) + 1) * 16;
    if (padded_len > out_max) return -1;

    /* Build padded plaintext block-by-block */
    uint8_t prev[16];
    for (int i = 0; i < 16; i++) prev[i] = iv[i];

    size_t offset = 0;
    while (offset < padded_len) {
        uint8_t block[16];
        for (int i = 0; i < 16; i++) {
            size_t idx = offset + (size_t)i;
            if (idx < pt_len)
                block[i] = pt[idx];
            else
                block[i] = (uint8_t)(padded_len - pt_len); /* PKCS#7 pad byte */
        }

        /* CBC: XOR with previous ciphertext (or IV for first block) */
        for (int i = 0; i < 16; i++) block[i] ^= prev[i];

        aes256_encrypt_block(key, block, ct + offset);

        for (int i = 0; i < 16; i++) prev[i] = ct[offset + i];
        offset += 16;
    }

    return (int)padded_len;
}

int aes256_cbc_decrypt(const uint8_t key[32], const uint8_t iv[16],
                       const void* ciphertext, size_t ct_len,
                       void* out_buf, size_t out_max)
{
    if (ct_len == 0 || ct_len % 16 != 0) return -1;
    if (ct_len > out_max) return -1;

    const uint8_t* ct = (const uint8_t*)ciphertext;
    uint8_t* pt = (uint8_t*)out_buf;

    uint8_t prev[16];
    for (int i = 0; i < 16; i++) prev[i] = iv[i];

    for (size_t offset = 0; offset < ct_len; offset += 16) {
        uint8_t block[16];
        aes256_decrypt_block(key, ct + offset, block);

        /* CBC: XOR with previous ciphertext */
        for (int i = 0; i < 16; i++)
            pt[offset + i] = block[i] ^ prev[i];

        for (int i = 0; i < 16; i++)
            prev[i] = ct[offset + i];
    }

    /* Remove PKCS#7 padding */
    uint8_t pad_val = pt[ct_len - 1];
    if (pad_val == 0 || pad_val > 16) return -1; /* Invalid padding */

    /* Verify all padding bytes */
    for (int i = 0; i < pad_val; i++) {
        if (pt[ct_len - 1 - i] != pad_val) return -1;
    }

    return (int)(ct_len - pad_val);
}
